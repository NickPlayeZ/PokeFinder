...
{
    return header.size() - (showPhenomenon ? 0 : 1) - (showMovingTrigger ? 0 : 1);
}
...
QVariant WildGeneratorModel5::data(const QModelIndex &index, int role) const
{
    ...
    if (role == Qt::DisplayRole)
    {
        ...
        int lastMetadataColumn = 1;
        if (showPhenomenon)
        {
            lastMetadataColumn = 2;
        }
        if (showMovingTrigger)
        {
            lastMetadataColumn = 3;
        }
        if (!state.isValid() && column > lastMetadataColumn)
        {
            return "-";
        }
...
void WildGeneratorModel5::setShowPhenomenon(bool flag)
{
    showPhenomenon = flag;
    emit headerDataChanged(Qt::Horizontal, 0, columnCount());
    emit layoutChanged();
...
QVariant WildSearcherModel5::data(const QModelIndex &index, int role) const
{
    if (role == Qt::DisplayRole)
    {
        ...
        int column = index.column();
        if (!showPhenomenon && column >= 3)
        {
            column++;
        }
        if (!showMovingTrigger && column >= 4)
        {
            column++;
        }
...
QVariant WildSearcherModel5::headerData(int section, Qt::Orientation orientation, int role) const
{
    if (role == Qt::DisplayRole && orientation == Qt::Horizontal)
    {
        if (!showPhenomenon && section >= 3)
        {
            section++;
        }
        if (!showMovingTrigger && section >= 4)
        {
            section++;
        }
...
void WildSearcherModel5::setShowPhenomenon(bool flag)
{
    showPhenomenon = flag;
    emit headerDataChanged(Qt::Horizontal, 0, columnCount());
    emit layoutChanged();
